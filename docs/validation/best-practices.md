...
description: Best practices for KosmoJS validation including centralized domain types, liberal use of TRefine, valid-state-only types, response validation for external data, and consistent error handling.
head:
  - - meta
    - name: keywords
...
Keep the `core/api/errors.ts` file updated to match your application's error handling conventions.
