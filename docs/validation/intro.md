  ...
  - - meta
    - name: keywords
      content: runtime validation, typebox, json schema, runtype validation,
        typescript validation, type safety, end-to-end validation
---
...
you write the type and the schema is derived from it.
...
also produce validation logic that runs when requests arrive,
...
Fetch clients validate request data on the client side before making any network request.
...
then AOT compilation to emit high-performance validation routines in your `lib` directory.
...
The derived code lives in `lib`, keeping your source directories focused on business logic.
At production build time it's bundled like any other dependency.

You don't need to read or understand the derived code to use validation.
