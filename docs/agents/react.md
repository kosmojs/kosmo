...
description: The complete React surface of KosmoJS on one page - configuration, routing,
    loaders, layouts, error handling, SSR and streaming, SSG, TanStack Query, the typed Link
    and templates, with every snippet in React's own idiom and no other framework's code.
head:
  - - meta
    - name: keywords
      content: react, kosmojs react, react-router, useLoaderData, Outlet, loader export,
        layout.tsx, app.tsx, entry/client, entry/server, renderToStream, defineStaticParams,
        tanstack react query, ErrorBoundary, Link component, no _/use in react
---

A folder with `frontend: { stack: "react" }`. Pages are `.tsx`, layouts are `layout.tsx`.

This page is the **complete React surface** of KosmoJS, written to be read on its own:
every snippet on it is React, and no other framework's code appears anywhere on the page.
Links go only to framework-agnostic pages; everything React-specific is inlined here.

Routing conventions, validation and the typed fetch clients behave the same for every
frontend. What this page fixes is the idiom: which router hooks exist, how children
render, where the loader lives, and what SSR and SSG look like in React.

## What the folder contains

Creating a source folder with a React frontend seeds a small, fixed set of files.
...
├── router.ts             -> routerFactory: routes -> React Router
│
├── components/
...
A React folder ignores other frameworks' files - a stray `.vue` or `.svelte` page is
never picked up.

## Configuration

The folder's `kosmo.config.ts` declares what the folder is. A typical React folder:
...
`"react"`. A bare name runs `@vitejs/plugin-react` with defaults. To configure the
...
**Never also list the React plugin in `viteConfig.plugins`** - the stack plugin reaches
...
depth` on the `defineConfig(...)` call - plugins that return arrays (Tailwind among them)
push Vite's recursive `PluginOption` past TypeScript's comparison limit. It is not needed
otherwise. A handful of Vite keys are not accepted, because KosmoJS derives them from the
folder layout: `root`, `base`, `cacheDir`, `mode`, `builder`, `future`, `legacy`.
...
React-specific support limits:

- **Mixed segments** (`files/[name].[ext]`) - React Router supports the `.ext` suffix
form only. Prefer simple segments on the page side.
- **Power syntax** (raw `path-to-regexp` patterns) - not supported on any frontend.

Route definitions are derived into `lib/` and handed to React Router as plain,
framework-native route objects - there is no route tree to register, and everything
React Router documents keeps working: nested layouts, lazy loading, loaders,
`errorElement`. All page components are **lazy-loaded** by default, fetched on demand
per navigation. [How&nbsp;routing&nbsp;derives&nbsp;›](/routing/intro.md) ·
[Frontend&nbsp;routing&nbsp;›](/frontend/routing.md)

## Pages

A page default-exports a component - a **named function**, never an anonymous arrow,
which breaks Vite's Fast Refresh:

```tsx [pages/users/index.tsx]
// React: pages/users/index.tsx
...
files only and never overwrites content. In containers and CI, where the file
watcher can behave clunky, create the empty files and run the build command - it
...
| search params | `useSearchParams()` from `react-router` |
...
`routerFactory` hands your `app` plus the derived routes to React Router, returning
...
`__KOSMO_HYDRATION_BOOL__` flag the server injects and picks `hydrate()` when SSR markup
...
A page exports `loader`, and React Router runs it at initial load, on link hover and on
navigation - so the data is there before the component renders, with no route-level
spinner. `useLoaderData` retrieves the already-fetched result, no duplicate request:
...
There is **no built-in loader cache** and no staleness model - React Router reuses the
in-flight result for that navigation, and real caching is TanStack Query's job. There is
also no proprietary `beforeLoad`-style hook: React Router's own primitives are untouched.

### Typing the result - `ResponseT`

Awaiting a fetch-client method already gives a typed result, so most call sites need no
annotation. `useLoaderData()` is the untyped boundary - for it, and for props, stores and
shared helpers, import the `ResponseT` map from `_/fetch`, keyed by route name then
method:

```ts [pages/users/[id]/helpers.ts]
// React: pages/users/[id]/helpers.ts
...
An entry exists in `ResponseT` **only when the handler declares a `response`** - without
one, the client method returns `Promise<unknown>` and there is no entry. Declaring
`response: [200, "json", T]` on the API handler is the one line that fixes it.
Variants without a body (`| [409]`) drop out of the union.

### Suspense

The `loader`/`useLoaderData` pattern resolves **before** render and does not suspend, so
route-level data needs no `<Suspense>` boundary. You only need one when you reach for
`React.lazy` or a promise-throwing `use()`. KosmoJS ships no app-wide boundary on
purpose - one boundary over everything collapses the whole page to a single fallback -
so if you add one, scope it to the subtree that actually suspends.
...
A layout is route-level, so it uses the same `loader` a page does. React scopes
structurally - each route (layouts included) owns its `loader`, and `useLoaderData()`
...
`layout.tsx` does**. It never appears in the typed `Link` route map, is lazy-loaded on
the client and imported eagerly into the SSR bundle.
...
a route that exists but has nothing to show branches in the component, and API 404s are
...
When a component throws while rendering, an error boundary catches it and swaps in
fallback UI. KosmoJS ships no boundary of its own - place React's native one in a
layout, so it covers that subtree and leaves siblings unaffected:
...
`react-error-boundary`'s `<ErrorBoundary fallbackRender={...}>` works identically if
you prefer a dependency over the class.

One SSR caveat: for a synchronous throw at the top of a component's render, **React
never renders the boundary's fallback HTML on the server** - the throw escapes the
render call. String-rendered routes recover by falling back to CSR (see below), and the
boundary works normally once the app hydrates; just don't lean on it to turn a hard
server-render failure into server-rendered fallback markup.

### Loader errors

A fetch call inside a route `loader` surfaces through React Router's own route-error
channel - the route's `errorElement`, read with `useRouteError()` - rendered the same on
the server render and after hydration. Handle loader failures there, **not** with a
`try`/`catch` inside the loader: catching inside hides the failure from the machinery
...
## Fetch clients on a React page

The typed fetch clients are framework-agnostic - typed params, payload and response, with
...
A fetch in `useEffect` runs after hydration, in the browser, over the network.
...
server-rendered, run `pnpm preview` - see [Production preview](#production-preview) below.
...
React supports stream rendering. Every route defaults to `"string"`; opt routes in by
...
as an empty shell in production means a failing call during render, so check the server
log before the client. The `onError` hook above turns that log line into a monitoring
event; it reports only.
- **Streamed routes do not recover.** The shell is on the wire before the fetch fails.
React contains the failure through the enclosing `<Suspense>` boundary: the stream
survives, the server emits the suspense fallback, and the client retries the subtree,
where an error boundary takes over. A streamed page must be able to render something
for every state its data can be in.
...
`useEffect` or guard with `typeof window !== "undefined"`. If a route misbehaves only
...
so the build machine needs the access production has (database, CMS), which is why SSG
belongs in CI. The output is **all-or-nothing**: one failed page and nothing is
...
A dev-only nuisance: a non-component export in a page file makes React Fast Refresh
fall back to a full reload for that file. The build is unaffected.
...
Enabling it deploys the `_/query` runtime and swaps `_/app` for a provider that supplies
...
To render a page's data on the server and hydrate it warm, use TanStack's own
primitives - prefetch in the loader, `dehydrate`, wrap the page in `HydrationBoundary`.
The one KosmoJS-specific detail: get the request-scoped client from `getQueryClient()`,
...
The default `components/Link.tsx` wraps React Router's link with compile-time route
validation. `to` takes a tuple of route name then params in path order, plus an
optional `query` prop:

```tsx [components/Menu.tsx]
// React: components/Menu.tsx
...
`jsxImportSource: "react"`, the reserved `@/` `~/` `_/` path mappings and strict
...
is what puts the folder, its `lib/` output and the ambient declarations in scope. If you
must, carry `["./", "../../lib/<folder>/", "../../lib/*.d.ts"]` over first. Run checks
with [kosmo typecheck](/cli/typecheck.md) - neither the dev server nor the build
typechecks for you. [Project&nbsp;layout&nbsp;›](/essentials/project-structure.md)

## Worth knowing

- **Create page and layout files empty and let KosmoJS seed them** - seeding
writes the current boilerplate with nothing left to recall wrong. In containers and
CI, where file watchers can behave clunky, create the empty files and run the
build - it resolves routes with the same code, deterministically.
- Loaders resolve before render, so route-level data needs no `<Suspense>` boundary -
that is a SolidJS concern.
- Streaming SSR is available for React (SolidJS and Vue also stream; Svelte and MDX
render to string only).
- `useLoaderData()` is the one untyped boundary - annotate it; everywhere else the fetch
client's types flow through.
- The default stack plugin is `@vitejs/plugin-react`. Pass your own through
`stack: { name: "react", plugin: react({ ... }) }`, never through `viteConfig.plugins`.
- `_/use` does not resolve in React folders - `react-router` covers that ground.
