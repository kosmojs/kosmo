...
here and nowhere else; H3 has no canonical CORS package to reach for, so it is
typically a small hand-written middleware setting the headers on `event.res`. This
layer runs before the event is extended - no `event.validated`, no `event.metaparser`,
no `event.bodyparser`.

**Order matters**: `onError` goes first, or middleware registered before it sits
outside its reach.
