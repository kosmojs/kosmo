---
title: H3 backend
description: The complete H3 surface of KosmoJS on one page - configuration, endpoints,
    validation, the middleware ladder, error handling, dev hooks, deployment and typing,
    with every snippet in H3's own idiom and no other backend's code anywhere on the page.
head:
  - - meta
    - name: keywords
      content: h3, kosmojs h3, event.context, event.req.headers, event.res, event.validated,
        onError, api/app.ts, api/errors.ts, api/use.ts, api/dev.ts, DefaultContext,
        h3 middleware, h3 validation, h3 deployment, app.fetch, toNodeHandler
---

A folder with `backend: { stack: "h3" }`.

This page is the **complete H3 surface** of KosmoJS, written to be read on its own:
every snippet on it is H3, and no other backend's code appears anywhere on the page.
Links go only to framework-agnostic pages; everything H3-specific is inlined here.

Routing conventions, validation semantics, middleware composition and the typed fetch
clients are identical across backends. What this page fixes is the idiom: how the event
carries state, how errors attach, and how the app mounts.

The handler argument is an **event**, not a context - it is the same object H3 passes
everywhere, and KosmoJS adds `event.validated` plus the cached `event.bodyparser` /
`event.metaparser` parsers to it. Handlers written against `ctx` will typecheck, since
the parameter name is yours to choose, but `ctx.req.header(...)` is Hono's API and does
not exist on an H3 event - headers are read with `event.req.headers.get(...)`,
a Web `Headers` object.

## What the folder contains

Creating a source folder with an H3 backend seeds a small, fixed set of files.
...
└── api/
    ├── app.ts            -> builds the H3 instance - onError middleware, app middleware
    ...
    ├── env.d.ts          -> DefaultContext, custom UseSlots
...
The folder's `kosmo.config.ts` declares what the folder is. A backend-only H3 folder:
...
`"h3"`. The object form `{ name: "h3" }` exists for symmetry with the frontend block,
...
export default defineRoute<"users">(({ GET, POST, PUT, DELETE }) => [
  GET(async (event) => { /* retrieve */ }),
  POST(async (event) => { /* create */ }),
  PUT(async (event) => { /* update */ }),
  DELETE(async (event) => { /* delete */ }),
]);
```
...
normally on H3 (unlike Hono, whose router ignores it).

### Responding from a handler

Return the value - H3 serializes it, objects as JSON:

| Task | H3 |
|---|---|
| read a header | `event.req.headers.get("authorization")` |
| respond | `return value` - objects serialize as JSON |
| set a status | `event.res.status = 400` |
| carry per-request state | `event.context.x = value` |
| raw params | `event.context.params` - untyped strings |
| validated params | `event.validated.params` - typed, coerced, checked |
...
`event.validated.params` and the merged cascading `use.ts` context for the route.
...
What follows is the H3 view of it.
...
number from `event.validated.params`. `VRefine` is globally available, no import needed.
...
export default defineRoute<"users">(({ POST }) => [
  POST<{
    ...
    const { name, email, age } = event.validated.json;
    const { notify } = event.validated.query;
...
export default defineRoute<"users">(({ POST }) => [
  ...
  >(async (event) => {
    const { id, email } = event.validated.json;
    return { id, email };
  }),
]);
```
...
the runtime check - the handler then reads the body itself via `event.bodyparser`.
...
KosmoJS extends the H3 event with three additions, available in every route chain:

- `event.bodyparser` - `.json()` / `.form()` / `.raw()`, async, cached per request
- `event.metaparser` - `.params()` / `.query()` / `.headers()` / `.cookies()`, sync,
cached; `params()` and `query()` come back normalized (splats split into arrays, values
coerced)
- `event.validated` - the validated, typed result per declared target

The parsers matter in [edge middleware](#edge-middleware) and in a
[custom validator](#overriding-validation), where `event.validated` is not filled yet.
They are **not** available in `api/app.ts`, which runs before the event is extended -
read the request through H3's own API there.
...
State goes on `event.context`, by plain assignment. This is the place for work that
belongs to routes and wants the request already validated: loading the current user,
...
Routes underneath destructure `event.context`, fully typed - no imports, no type
arguments. The global `api/use.ts` is the exception: its `UseT` export
is ignored; global types come from `api/env.d.ts` instead. Inner `use.ts` files can
extend a parent's type:

```ts [api/users/account/use.ts]
// H3: api/users/account/use.ts
...
H3 ships fewer off-the-shelf middleware packages than Hono or Koa, so more of this layer
tends to be hand-written against `event.req` / `event.res`, wired through `use()`:
...
export default [
  use(
    async (event, next) => { /* global logger */ },
    { slot: "logger" },
  ),
];
```
...
export default defineRoute<"import">(({ POST, use }) => [
  ...
  POST(async (event) => {
    // event.validated.json is NOT set for an overridden target -
    // the parsers are cached, so asking again costs nothing
    const records = await event.bodyparser.raw<string>();
    return { imported: records.split("\n").length };
  }),
]);
```
...
// H3: api/webhooks/stripe/index.ts
...
extends the event; claiming it removes `event.metaparser`, `event.bodyparser` and
`event.validated` for everything downstream. Always prefix.
...
neither do preflights or `405`s. `event.validated` is empty here - read through
`event.metaparser` / `event.bodyparser` instead.

### App middleware

The outermost layer, and the only one KosmoJS doesn't compose. `api/app.ts` hands you
the native H3 instance, so anything H3 can do at app level goes here, written exactly as
H3's own docs describe:

```ts [api/app.ts]
<!--@include: @/parts/backend/middleware/cors.md#h3-->
```

> **Order matters**: `onError` goes first, or middleware registered before it sits outside its reach.
...
H3 has no canonical CORS package to reach for,
so it is typically a small hand-written middleware setting the headers on `event.res`.

> This layer runs before the event is extended - so no `event.validated`, no `event.metaparser`, no `event.bodyparser`.
...
**first** - in `api/app.ts` via `app.use(onError(defaultErrorHandler))`, so everything
registered after it runs inside its reach. The handler may return a
`Response`, a plain object or a string; branch on `event.url` when behavior should
differ per route:
...
It is a regular file you own - customize it freely.
...
not two arguments
- H3's own `HTTPError` - takes an object, `new HTTPError({ status: 401, message: "..." })`
...
change. The default `requestHandler` turns the H3 app into a Node listener with
`toNodeHandler` - note it takes the **app**, not `app.fetch`:
...
## Routing syntax support on H3

The three plain syntaxes - `[id]` required, `{id}` optional, `{...path}` splat -
work everywhere. The exotic ones vary:

- **Mixed segments** (`files/[name].[ext]`) - **partial** support on H3, with caveats.
Full support is Koa-only on the backend side; keep mixed segments simple here.
- **Power syntax** (raw `path-to-regexp v8` patterns like `book{-:id}-info`) - H3
**won't match** these routes at all. It is fully usable on Koa only.
...
```txt
dist/<folder>/api/
├── app.js       -> the H3 app instance, for custom mounting
...
For more control, mount the app factory yourself. On Node, `toNodeHandler` adapts the
app; on fetch-native runtimes, `app.fetch` is a Web Fetch API handler that plugs in
directly:

```js
// H3: mounting app.js on Node
...
```ts
// H3: mounting app.js on Bun
import app from "./dist/front/api/app.js";

Bun.serve({ port: 3000, fetch: app.fetch });
```

The same `app.fetch` is what wires the API into edge runtimes - there is no automatic
serverless packaging; you hand `app.fetch` to the runtime yourself.
...
H3 has a single context surface, so there is one interface to augment:
...
to run, usually in `api/use.ts` via `event.context.permissions = ...`.
...
it has to be written in H3's idiom:
...
export default defineRoute<"{{route.name}}">(({ GET, POST }) => [
  GET<{
    ...
    // list - return the value directly, objects serialize as JSON
    return [];
  }),
...
Handlers written against `ctx` will typecheck, since the name is yours to choose, but
`ctx.req.header(...)` is Hono's API and does not exist on an H3 event - headers are
read with `event.req.headers.get(...)`, a Web `Headers` object.
- App middleware in `api/app.ts` runs before the event is extended - no
`event.validated`, no parsers there.
- Two `HTTPError` classes exist: KosmoJS's takes a `[status, message]` tuple, H3's
takes a `{ status, message }` object. The default handler understands both.
- Explicit `HEAD` handlers run normally on H3; the `GET` fallback covers routes that
don't define one.
- Power syntax won't match on H3 at all - treat it as unavailable here.
