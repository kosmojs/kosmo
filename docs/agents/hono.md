---
title: Hono backend
description: The complete Hono surface of KosmoJS on one page - configuration, endpoints,
    validation, the middleware ladder, error handling, dev hooks, deployment and typing,
    with every snippet in Hono's own idiom and no other backend's code anywhere on the page.
head:
  - - meta
    - name: keywords
      content: hono, kosmojs hono, ctx.json, ctx.req.header, ctx.set, app.onError, HTTPException,
        api/app.ts, api/errors.ts, api/use.ts, api/dev.ts, DefaultVariables, DefaultBindings,
        hono middleware, hono validation, hono deployment, app.fetch, getRequestListener
---

A folder with `backend: { stack: "hono" }`.

This page is the **complete Hono surface** of KosmoJS, written to be read on its own:
every snippet on it is Hono, and no other backend's code appears anywhere on the page.
Links go only to framework-agnostic pages; everything Hono-specific is inlined here.

Routing conventions, validation semantics, middleware composition and the typed fetch
clients are identical across backends. What this page fixes is the idiom: how the context
reads and writes, how errors attach, and how the app mounts.

## What the folder contains

Creating a source folder with a Hono backend seeds a small, fixed set of files.
...
└── api/
    ├── app.ts            -> builds the Hono instance - error handler, app middleware
    ...
    ├── env.d.ts          -> DefaultVariables / DefaultBindings, custom UseSlots
...
The folder's `kosmo.config.ts` declares what the folder is. A backend-only Hono folder:
...
`"hono"`. The object form `{ name: "hono" }` exists for symmetry with the frontend block,
but backend stacks carry no Vite plugin, so the bare name is the usual form.

### backend.base - required

The URL prefix this folder's routes are served from - a **full path**, resolved on its own,
never joined onto a frontend base. A route's final URL is `backend.base` + route name:
...
Each `servers.url` carries the full prefix, origin plus `backend.base` - paths in the spec
are route names and never carry the prefix themselves.
[OpenAPI&nbsp;›](/openapi/intro.md) · [Config&nbsp;details&nbsp;›](/openapi/config.md)

### backend.alias

Serves an existing route at an additional public URL. Keys are **absolute** - not prefixed
by `backend.base` - and dynamic segments must match the target's parameter names exactly,
or the request 404s:
...
An alias is another entry pointing at the same handler - middleware, cascading middleware
and validation all come with it. It is not a redirect, and it does not touch the fetch
client or the OpenAPI spec, both keyed by the route's own name.
...
- **`fetch: true`** - derive typed [fetch clients](/fetch/intro.md) from this folder's routes.
- **`validation: true`** - runtime validators derived from your types. For anything beyond
on/off, pass an options object: `validationMessages` (global message overrides,
`node:util.format` placeholders), `customTypesImport` (a file mapping custom TypeBox types),
`refineTypeName` (renames `VRefine` if it collides), and `settings`
(`maxErrors`, `useEval: false` for strict CSP environments, `exactOptionalPropertyTypes`,
`immutableTypes`).
- **`typecheck: false`** - leave the folder out of [kosmo typecheck](/cli/typecheck.md) runs.
...
with the body dropped as the HTTP spec requires. **On Hono this fallback is the only
behavior**: Hono's router ignores any `HEAD` handler you define, so the dispatch to
`GET` always wins here - do not write an explicit `HEAD` builder expecting it to run.
...
| respond with JSON | `return ctx.json(value)` / `return ctx.json(value, 201)` |
| respond with text | `return ctx.text(message, status)` |
| carry per-request state | `ctx.set("key", value)` / `ctx.get("key")` |
| raw params | `ctx.req.param()` - untyped strings |
...
derived `RouteMap` in `lib/`, and that single lookup is what types `ctx.validated.params`
and the merged cascading `use.ts` context for the route. Because no runtime argument
carries it, TypeScript has nothing to infer it from, so the type argument is **required** -
and the seeded boilerplate already contains it.
...
What follows is the Hono view of it.
...
number from `ctx.validated.params`. `VRefine` is globally available, no import needed.
[Params&nbsp;details&nbsp;›](/validation/params.md) · [VRefine&nbsp;keywords&nbsp;›](/validation/refine.md)

### Payload

Metadata targets (`query`, `headers`, `cookies`) are valid on every method; body targets
(`json`, `form`, `raw`) on POST/PUT/PATCH only, one per handler. Coercion: `query` coerces
numbers and booleans, `params` numbers only, `json` carries both natively,
...
sent. Imported and generic types (`json: Payload<User>`) resolve fully - every referenced
type is traced, and validation rebuilds when a shared type changes.
...
export default defineRoute<"users">(({ GET }) => [
  ...
  },
  {
    ...
    return ctx.json(await loadUser());
  }),
]);
```
...
KosmoJS extends the Hono context with three additions, available in every route chain:

- `ctx.bodyparser` - `.json()` / `.form()` / `.raw()`, async, cached per request
- `ctx.metaparser` - `.params()` / `.query()` / `.headers()` / `.cookies()`, sync, cached;
`params()` and `query()` come back normalized (splats split into arrays, values coerced)
...
read the request through Hono's own API there.
...
user, permission checks, audit logging.

It is **skipped for non-route responses**: a preflight `OPTIONS`, or a `405`, is answered
before any route chain runs. Which makes it the wrong home for CORS - that belongs in
...
from `api/env.d.ts` instead. Inner `use.ts` files can extend a parent's type:

```ts [api/users/account/use.ts]
// Hono: api/users/account/use.ts
...
earlier middleware at that position, keeping the surrounding order. A global default with
a slot can be substituted per subtree or per route; one without a slot always runs and
cannot be overridden - which is what you want for a security check.
...
export default defineRoute<"import">(({ POST, use }) => [
  ...
  POST(async (ctx) => {
    ...
    return ctx.json({ imported: records.split("\n").length });
  }),
]);
```
...
// Hono: api/webhooks/stripe/index.ts
...
The outermost layer, and the only one KosmoJS doesn't compose. `api/app.ts` hands you the
native Hono instance, so anything Hono can do at app level goes here, written exactly as
...
Hono's `app.onError()` catches everything (`await next()` does **not** throw), and the
...
not two arguments
- Hono's own `HTTPException` - `new HTTPException(401, { message: "..." })`
...
`debug` also takes `"headline"` / `"methods"` / `"middleware"` / `"handler"` for targeted
output, or a function `debug(log, route)` for a custom logger. Named middleware functions
print by name; anonymous ones print their first line - name them.

## Routing syntax support on Hono

The three plain syntaxes - `[id]` required, `{id}` optional, `{...path}` splat -
work everywhere. The exotic ones vary:

- **Mixed segments** (`files/[name].[ext]`) - **partial** support on Hono, with caveats.
Full support is Koa-only on the backend side; keep mixed segments simple here.
- **Power syntax** (raw `path-to-regexp v8` patterns like `book{-:id}-info`) - Hono
**matches** these routes but registers the params under positional names such as `_0abc`
rather than the names you wrote. Avoid it on Hono; it is fully usable on Koa only.
...
`pnpm build` bundles the backend into `dist/<folder>/api/` as ESM with
sourcemaps - two entry points; pass folder names to build a subset:

```txt
dist/<folder>/api/
├── app.js       -> the Hono app instance, for custom mounting
...
For more control, mount the app factory yourself - `app.fetch` is a Web Fetch handler:

```js
// Hono: mounting app.js on Node
...
```ts
// Hono: mounting app.js on Bun
import app from "./dist/front/api/app.js";

Bun.serve({ port: 3000, fetch: app.fetch });
```

The same `app.fetch` is what wires the API into Cloudflare Workers and other edge
runtimes - there is no automatic serverless packaging; you hand `app.fetch` to the
runtime yourself.
...
export default defineRoute<
  ...
  { DB: D1Database },                        // Cloudflare binding, ctx.env.DB
>(({ GET }) => [
  GET(async (ctx) => {
    const { id } = ctx.validated.params;
...
pattern wins, in written order, `*` matching one level and `**` any depth. Templates fill
**blank files only**; existing content is never overwritten. The template is a string
(rendered with Handlebars against `{ route }`) or a function of the route, and it has to
be written in Hono's idiom:
...
export default defineRoute<"{{route.name}}">(({ GET, POST }) => [
  ...
  POST<{
    ...
    // create
    return ctx.json({} as Resource, 201);
  }),
]);`;
...
- **Create route files empty and let KosmoJS seed them** - seeding writes the
current boilerplate, imports and type arguments included, with nothing left to
recall wrong. In containers and CI, where file watchers can behave clunky, create
the empty files and run the build - it resolves routes with the same code,
deterministically.
- App middleware in `api/app.ts` runs before the context is extended - no
`ctx.validated`, no parsers there.
- Third-party Hono middleware goes on the native instance in `api/app.ts`, or into
`use()` when it should participate in the route chain.
- Hono's router ignores explicit `HEAD` handlers - the `GET` fallback always serves
HEAD requests.
- `HTTPError` from `@kosmojs/core/errors` takes a `[status, message]` tuple; Hono's
`HTTPException` takes `(status, { message })`.
- Power syntax matches on Hono but renames params to positional `_0abc` forms - treat
