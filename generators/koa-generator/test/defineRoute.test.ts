...
describe("defineRoute", () => {
  describe("use", () => {
    test("accepts single middleware", () => {
      async function mw() {}
      const [stack] = defineRoute(({ use }) => [use(mw)]) as unknown as Array<
    ...
    test("accepts middleware array", () => {
      ...
      const [stack] = defineRoute(({ use }) => [
        use([mw1, mw2]),
      ]) as unknown as Array<MiddlewareDefinition<ParameterizedMiddleware>>;
      expect(stack.kind).toEqual("middleware");
    ...
    test("accepts slot option", () => {
      ...
      expect(stack.options?.slot).toEqual("edge");
    });
