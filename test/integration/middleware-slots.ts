...
/**
 * The only per-backend differences. Everything else the generated middleware does -
 * reading the request, recording the chain, short-circuiting with a status -
 * goes through the framework-agnostic KosmoJS API.
 *
 * Denying short-circuits by sending a response rather than throwing:
 * the seeded `api/errors.ts` files do not agree on how a thrown value becomes a status,
 * and that difference is not what these tests are about.
 * */
...
export const createTests = async (backend: Backend) => {
  ...
  /**
   * Global middleware: two edge slots plus an unslotted entry,
   * so every test can tell an edge position from the ordinary global one.
   * */
  await writeFile(
  ...
  const routes: Array<[name: string, body: string]> = [
     ...
     * above it answers the question the feature exists for:
     * which status does a request that is both unauthenticated and invalid get?
     * */
    [
      "gated",
      `use(async (${ctx}, next) => {
  ...
  /**
   * Each matrix route declares a numeric `id`, and every request below sends a non-numeric one.
   * A surviving built-in `validate:params` would answer 400,
  ...
  const tests: Array<{ name: string; runner: TestFunction }> = [
    ...
    },
    {
      ...
      async runner({ expect }) {
         ...
         * override and the built-in validator - and is then filtered out of a GET,
         * leaving the slot with nothing in it.
         *
         * The route declares a numeric id and the request sends "abc":
         * a 200 means nothing validated it. Pinning current behaviour,
         * not endorsing it - scoping an override silently drops validation elsewhere.
         * */
        const { status, trace } = await matrixTrace("scoped/probe/abc");

        // nothing holds validate:params - not the cascading override that claimed it,
        // not the global one it displaced, not the built-in
